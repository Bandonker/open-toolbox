...
const PACKAGES = [
  ...
  },
  {
    ...
    rewrites: {},
    description:
      "Trim stale tool output from the context sent to the model on every request (session.hook context), saving tokens without touching the transcript on disk.",
    keywords: ["context", "tokens", "compaction", "pruner"],
    tools: [
    ...
    config: [
      ...
      ["OPENCODE_CONTEXT_PRUNER_IGNORE", "(none)", "Comma-separated tools to never prune"],
